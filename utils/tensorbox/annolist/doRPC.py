    ...
    def decreaseScore(self, score):
        ...
        if capacity_change:
            ...
            self.ignore_flow = sum(
                [self.F[i][self.a - 1] * self.ignore[i - 1 - self.n] for i in range(1 + self.n, 1 + self.n + self.m)])
...
def asort(idlGT, idlDet, minWidth, minHeight, style, minCover, minOverlap, maxDistance, maxWidth=float('inf'),
          maxHeight=float('inf')):
    ...
    for x, anno in enumerate(idlGT):
        ...
        validGTRects = []
        for j in anno.rects:
            if (j.width() >= minWidth) and (j.height() >= minHeight) and (j.width() <= maxWidth) and (
                j.height() <= maxHeight):
...
# MA: full version
def comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, minWidth=0, minHeight=0, maxWidth=float('inf'),
                                maxHeight=float('inf'),
                                matchingStyle=1, minCover=0.5, minOverlap=0.5, maxDistance=0.5, ignoreOverlap=0.9,
                                verbose=False):
    ...
    for i in range(len(annoIDL)):
        ...
        if (not imageFound):
            print("No annotation/detection pair found for: " + annoIDL[i].imageName + " frame: " + str(
                annoIDL[i].frameNr))
            missingFrames += 1
            continue

        graphs.append(
            AnnoGraph(annoIDL[i], detIDL[filterIndex], ignoreIDL[i], matchingStyle, minCover, minOverlap, maxDistance,
                      ignoreOverlap))
    ...
    if (options.dropFirst == True):
        ...
        for i, anno in enumerate(detIDL):
            if (i > 1 and detIDL[i].frameNr == detIDL[i - 1].frameNr + 1 and detIDL[i].frameNr == detIDL[
                    i - 2].frameNr + 2 and detIDL[i].frameNr == detIDL[i - 3].frameNr + 3 and detIDL[i].frameNr ==
                    detIDL[i - 4].frameNr + 4):
                newIDL.append(anno)
        detIDL = newIDL
