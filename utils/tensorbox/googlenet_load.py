...
def init(H, config=None):
    ...
    graph_def_orig_file = './data/models/googlenet.pb'
...
def model_v2(x, googlenet, H):
    ...
    for op in reuse_ops:
        if is_early_loss(op.name):
            ...
            pool_op = tf.nn.avg_pool(T['mixed5b'], ksize=[1, H['arch']['grid_height'], H['arch']['grid_width'], 1],
                                     strides=[1, 1, 1, 1], padding='VALID', name=op.name)
            T[op.name] = pool_op
